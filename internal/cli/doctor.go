...
// runDoctor loads the configured exports, prints what was loaded and reports
// graph problems. Cycles among open beads are errors (they can never be
...
func runDoctor(args []string, stdout, stderr io.Writer) int {
	...
	if fs.NArg() > 0 {
		fmt.Fprintf(stderr, "beadline doctor: unexpected argument %q\n", fs.Arg(0))
		return ExitUsage
	}
	...
	for _, r := range rep.Repos {
		fmt.Fprintf(stdout, "repo %s: %s from %s%s\n", r.Repo, plural(r.Issues, "issue"), r.Source, skipped(r))
	}
	gates := 0
